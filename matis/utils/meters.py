...
class SurgeryMeter(object):
    ...
    def __init__(self, overall_iters, cfg, mode):
        ...
        #TODO: VERIFICAR COMO MANEJAR LAS MULTIPLES TAREAS
        ...
        # TODO: Usar la función de evaluación y llevar a las anotaciones
        self.groundtruth = cfg.AVA.COCO_ANN_DIR
        self.features = cfg.MASKFORMER.FEATURES_VAL
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        if (cur_iter) % self.cfg.LOG_PERIOD != 0:
            return
    ...
    def update_stats(self, preds, boxes, d_names, names, final_loss= None, losses=None, lr=None):
        """
        TODO: update docs
    ...
    def finalize_metrics(self, log=True):
        ...
        if self.mode == 'train':
            ...
            for idx, task in enumerate(self.tasks):
                ...
                self.full_map[task] = psiava_eval.main(self.groundtruth, out_name[task], [task], self.cfg.FOLD, self.cfg.TRAIN.DATASET)
                if log:
                    stats = {"mode": self.mode,"task":task, "loss": task_loss[idx], "map": self.full_map[task][0], "met1": self.full_map[task][1], "met2": self.full_map[task][2]}
                    logging.log_json_stats(stats)
            if log:
                stats = {"mode": self.mode, "mean map": np.mean([u[0] for u in list(self.full_map.values())]), "final loss": self.final_loss}
                logging.log_json_stats(stats)
        else:
            for idx, task in enumerate(self.tasks):
                if task != 'features' or len(task['features'])>0:
                    out_name[task] = self.save_json(task, self.all_preds[task], self.all_boxes,  self.all_detect_names, self.all_names)
                if task!= 'features':
                    self.full_map[task] = psiava_eval.main(self.groundtruth, out_name[task], [task], self.features, self.cfg.FOLD, self.cfg.TRAIN.DATASET)
                    if log:
                        stats = {"mode": self.mode,"task":task, "map": self.full_map[task][0], "met1": self.full_map[task][1], "met2": self.full_map[task][2]}
                        logging.log_json_stats(stats)
            if log:
                stats = {"mode": self.mode, "mean map": np.mean([u[0] for u in list(self.full_map.values())])}
                logging.log_json_stats(stats)
        
        # return self.full_map, np.mean(list([u[0] for u in list(self.full_map.values())])), out_name
        return self.full_map, self.full_map['tools'][0] , out_name
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        """
        if self.mode in ["val", "test"]:
            metrics_val, mean_metric, out_name = self.finalize_metrics(log=True)
            stats = {
            ...
            # TODO: add overall task metric
            logging.log_json_stats(stats)
            
            return metrics_val, mean_metric, out_name
