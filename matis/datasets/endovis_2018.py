...
from . import endovis_2018_helper as ava_helper
...
class Endovis_2018(torch.utils.data.Dataset):
    """
    SAR-RARP50 Dataset
    """
    ...
    def print_summary(self):
        logger.info("=== ENDOVIS 2018 dataset summary ===")
    ...
    def __getitem__(self, idx):
        ...
        assert path_complete_name==self._image_paths[video_idx][sec], complete_name + ' & ' + self._image_paths[video_idx][sec]
            ...
            for b_idx, box_labels in enumerate(clip_label_list):
                ...
                if self.cfg.MASKFORMER.ENABLE:
                    ...
                    if box_labels[0]!=[0,0,0,0]:
                        # breakpoint()
                        assert faster_box_key in box_features, 'La caja {} no esta {},{}'.format(faster_box_key,video_name,sec)
                        faster_features.append(np.array(box_features[faster_box_key]))
                    else:
                        faster_features.append(np.zeros(256)) #np.random.uniform(-1.5,1.5,256))
